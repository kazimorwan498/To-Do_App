...
addBtn.addEventListener("click", (e) => {
	...
	listOption.innerHTML = `<span>${todoInput2}</span> ${taskInnerBtn}`;

	todoInput.value = "";
	todoLs.prepend(listOption);
...
function doneTask(items) {
	...
	if (textSpan.classList.contains("line-through")) {
		doneBtn.innerHTML = restartBtnSvg;

		const editBtn = items.querySelector(".editBtn");
		if (editBtn) {
			editBtn.classList.add("pointer-events-none");
		}
	} else {
		doneBtn.innerHTML = doneBtnSvg;
		if (items.querySelector(".editBtn")) {
			items
				.querySelector(".editBtn")
